...
def hub(vec, delta):
    return 1/2/delta*norm(vec) if norm(vec) <= delta else norm(vec) - delta/2

# 计算方程的值


def function1(X, A, labda, delta):
    num_points = len(X)
    Xn = np.tile(X, (num_points, 1, 1))
    XnT = np.transpose(Xn, axes=(1, 0, 2))
    Xnn = np.triu(Xn - XnT)
    return 1/2*((X-A)**2).sum() + labda*hub(Xnn, delta)
...
def grad_hub_matrix(X, delta, A, labda):
    ...
    XnT = np.transpose(Xn, axes=(1, 0, 2))
    mat = Xn - XnT
    tool_mat = np.triu(-np.ones((num_points, num_points))) + \
        np.tril(np.ones((num_points, num_points)))
